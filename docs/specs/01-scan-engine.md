...
| SCAN-FR-003 | 文件尺寸与修改时间**必须**通过 [`FSCTL_GET_NTFS_FILE_RECORD`](https://learn.microsoft.com/windows/win32/api/winioctl/ni-winioctl-fsctl_get_ntfs_file_record) 批量读取 MFT 文件记录，解析 `$STANDARD_INFORMATION`/`$FILE_NAME`/`$DATA` 属性获得逻辑大小、分配大小与 `LastWriteTime`（UTC）；该 FSCTL 顺序读取时等效于顺序扫描 $MFT，WizTree 同类实现先例见 06 F-03 | MUST |
...
| SCAN-FR-021 | 全量枚举完成后**必须**做一次自底向上聚合：每目录的累计逻辑/分配大小、文件数、子树内最大 `LastWriteTimeUtc` | MUST |
| SCAN-FR-022 | **必须**提供查询：Top-N 大文件、Top-N 大文件夹、目录子树钻取（treemap 数据源）、**前缀/glob 路径匹配（供规则引擎与 Adapter 展开）**；全部在内存索引上执行，**禁止**为已知 target 子树回退目录遍历 | MUST |
| SCAN-FR-023 | 查询与增量更新**必须**并发安全（快照或 `ReaderWriterLockSlim`） | MUST |
| SCAN-FR-024 | Top-N 大目录结果**必须**折叠祖先链：当某目录的单一子目录占其子树占用 ≥ 支配阈值（默认 0.7）时，抑制该父目录，仅保留承载信息的子目录；扫描根目录始终排除 | MUST |
| SCAN-FR-025 | **必须**提供条件搜索：按类型（文件/目录/全部）、最小占用大小、名称片段（不区分大小写）过滤，返回按占用降序的 Top-N；目录侧沿用 SCAN-FR-024 折叠语义 | MUST |
...
| ID | 需求 | 级别 |
|----|------|------|
| SCAN-FR-026 | 索引**必须**为每个文件/目录节点存储 `LastWriteTimeUtc`（来自 MFT `$STANDARD_INFORMATION`；解析失败时为 `DateTime.MinValue`） | MUST |
| SCAN-FR-027 | 聚合阶段**必须**为每个目录节点计算子树 `MaxLastWriteTimeUtc`（子树内所有文件/目录 LastWrite 的最大值） | MUST |
| SCAN-FR-028 | 引擎**必须**暴露 `IIndexQuery` 只读接口，至少含：`TryResolvePrefix`、`GetSubtreeStats`、`FindDirectoriesByName`、`FindFilesByExtension`、`ExpandGlob`、`SubtreeContainsExtension`；实现**必须**仅读内存索引 | MUST |
| SCAN-FR-029 | 落盘缓存**必须**携带 schema 版本号；v1→v2 不兼容时 `Load` 返回失败并触发全量重建（见 ADR-001）；v2 追加 `lastWriteUtc[]` 并纳入 `XxHash64` 校验 | MUST |

#### SCAN-FR-022 / SCAN-FR-028 — Glob 语义

| 模式 | 语义 | 示例 |
|------|------|------|
| 前缀 | 绝对路径前缀匹配（大小写不敏感） | `C:\Users\*\AppData\Local` 经环境符号展开后为前缀 |
| 单段 `*` | 匹配一层目录名 | `...\User Data\*\Cache` |
| `**` | 规则 target 递归语义；查询 API 以 `maxDepth` 或 `recursive` 参数表达 | `${LOCALAPPDATA}/**` + `maxDepth=8` |

子树查询性能**必须**满足 SCAN-NFR-002（400 万节点卷上单次查询 < 100 ms）。
